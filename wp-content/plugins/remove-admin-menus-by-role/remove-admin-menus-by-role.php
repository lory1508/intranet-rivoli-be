...
function remove_menu_admin_free_desinstall() {
	...
	if(!is_plugin_active('remove-admin-menus-by-role-pro/remove-admin-menus-by-role-pro.php'))
	{
		...
		$wpdb->query("DROP TABLE ".$wpdb->prefix . "remove_menu_admin_profiles".";");
	}
...

	{
...

            {
...

            	{
            		...
            		foreach($submenus_hidden as $c_submenu)

	            	{

	            		$data = explode('|', $c_submenu); //$submenu = menu|submenu

	            		//special case for Appearance / Customize (url parameter cause bug)

						if(preg_match("^customize\.php(.*)^", $data[1]))

						{

							//looking fo the real url of custimize submenus

							global $submenu;

							foreach($submenu['themes.php'] as $real_sm)

							{

								if(preg_match("^customize\.php(.*)^", $real_sm[2]))
...

		{
...

			{
				...
				foreach($_POST['menus_hidden'] as $key => $value)
					...
					$_POST['menus_hidden'][$key] = wp_kses_post($_POST['menus_hidden'][$key]);
					//$_POST['menus_hidden'][$key] = $_POST['menus_hidden'][$key];
			}
...

			{
				...
				foreach($_POST['submenus_hidden'] as $key => $value)
					...
					//$_POST['submenus_hidden'][$key] = $_POST['submenus_hidden'][$key];
					$_POST['submenus_hidden'][$key] = wp_kses_post($_POST['submenus_hidden'][$key]);
			...
			$query = $wpdb->query($wpdb->prepare( "REPLACE INTO ".$wpdb->prefix . "remove_menu_admin_profiles (`id`, `name`, `roles`, `menus_hidden`, `submenus_hidden`, `active`) VALUES (1, 'Profil', %s, %s, %s, %d)", serialize($_POST['roles']), serialize($_POST['menus_hidden']), serialize($_POST['submenus_hidden']), $_POST['active']));
...
function get_remove_menu_admin_profile_free($id=1)
{
	...
	//$query = $wpdb->prepare("SELECT roles, menus_hidden, submenus_hidden FROM ".$wpdb->prefix."remove_menu_admin_profiles." WHERE id=%d", $id);

	return $wpdb->get_row($wpdb->prepare("SELECT roles, menus_hidden, submenus_hidden FROM ".$wpdb->prefix."remove_menu_admin_profiles WHERE id=%d", $id));
...
function remove_menus_by_roles_textdomain() {
    ...
    load_plugin_textdomain( 'remove-admin-menus-by-role');
